...
from ModelMerge.src.ModelMerge.models import (
    chatgpt,
    claude,
    groq,
    claude3,
...
PREFERENCES = {
    ...
    "FILE_UPLOAD_MESS": (os.environ.get("FILE_UPLOAD_MESS", "True") == "False")
    == False,
...
systemprompt = os.environ.get(
    "SYSTEMPROMPT", prompt.system_prompt.format(LANGUAGE, Current_Date)
)
claude_systemprompt = os.environ.get(
    "SYSTEMPROMPT", prompt.claude_system_prompt.format(LANGUAGE)
)
...
class UserConfig:
    ...
    def extract_plugins_config(self, user_id=None):
        ...
        plugins_config = {
            key: value for key, value in user_data.items() if key in self.plugins
        }
...
Users = UserConfig(
    ...
    claude_systemprompt=claude_systemprompt,
)
...
ChatGPTbot, SummaryBot, claudeBot, claude3Bot, groqBot, gemini_Bot, whisperBot = (
    None,
    None,
    None,
    None,
...
def update_language_status(language, chat_id=None):
    ...
    try:
        ...
        claude_systemprompt = claude_systemprompt.replace(
            LAST_LANGUAGE, Users.get_config(chat_id, "language")
        )
...
def get_local_version_info():
    ...
    result = subprocess.run(
        ["git", "-C", current_directory, "log", "-1"], stdout=subprocess.PIPE
    )
...
def get_remote_version_info():
    current_directory = os.path.dirname(os.path.abspath(__file__))
    result = subprocess.run(
        ["git", "-C", current_directory, "ls-remote", "origin", "HEAD"],
...
def create_buttons(
    ...
    # 过滤出长度小于15的字符串
    filtered_strings1 = [
        s for s in strings if len(delete_model_digit_tail(s.split("-"))) <= 14
    ]
    filtered_strings2 = [
        s for s in strings if len(delete_model_digit_tail(s.split("-"))) > 14
    ]
    ...
    for string in filtered_strings1:
        if plugins_status:
            button = InlineKeyboardButton(
                f"{get_status(chatid, string)}{button_text[string][lang]}",
                callback_data=string + Suffix,
            )
        else:
            button = InlineKeyboardButton(
                delete_model_digit_tail(string.split("-")),
                callback_data=string + Suffix,
            )
        temp.append(button)
...
if CUSTOM_MODELS_LIST:
    ...
    initial_model.extend(
        [
            ...
            if model not in initial_model and model[0] != "-"
        ]
    )
...
def update_first_buttons_message(chatid=None):
    ...
    first_buttons = [
        [
            InlineKeyboardButton(
                strings["button_change_model"][lang], callback_data="MODELS"
            ),
            InlineKeyboardButton(
                strings["button_preferences"][lang], callback_data="PREFERENCES"
            ),
        ],
        [
            InlineKeyboardButton(
                strings["button_language"][lang], callback_data="LANGUAGE"
            ),
            InlineKeyboardButton(
                strings["button_plugins"][lang], callback_data="PLUGINS"
            ),
        ],
    ]
